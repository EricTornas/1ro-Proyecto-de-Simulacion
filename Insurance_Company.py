...
def simulate_insurance_risk_model(a0, n0,  param_lambda,  param_ni,  param_mi, c, T, F):
    ...
    while tE <= T:
        ...
        J = np.random.choice([1,  2,  3], p=[param_ni / (param_ni + n *  param_mi + n *  param_lambda), (n *  param_mi) / (param_ni + n *  param_mi + n *  param_lambda), (n *  param_lambda) / (param_ni + n *  param_mi + n *  param_lambda)])
        ...
        if J ==  1: # Nuevo asegurado
            ...
            (cant_claims, amount) = claims[math.floor(t)] # Actualizamos la cantidad de Reclamaciones en una unidad temporal y el monto total de esas reclamaciones para esa unidad temporal
            a -= Y
            cant_claims+=1
            amount+= Y
